...
def collision_checker(robot_1l, robot_2l, robot_3l, goal_states):
    ...
    print(f"Reached function {robot_1.current_state}, {robot_2.current_state}, {robot_3.current_state}")
...
def main():
    ...
    start_states = [[4, 6], [6, 4], [17, 17]]                                    # Defining Start states
    goal_states = [[8, 8], [8, 10], [13, 16]]                                      # Defining Goal states
    ...
    first = min(list_robots, key = lambda x : x[1])
    list_robots.remove(first)
    second = min(list_robots, key = lambda x : x[1])
