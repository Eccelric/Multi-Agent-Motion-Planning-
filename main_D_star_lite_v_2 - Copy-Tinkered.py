...
def collision_checker(robot_1l, robot_2l, robot_3l, goal_states, to_change_coords_2, to_change_coords_3):
    robot_1, id_1, flag_1 = robot_1l
    robot_2, id_2, flag_2 = robot_2l
    robot_3, id_3, flag_3 = robot_3l
    print("collision checker")
    if len(to_change_coords_2) != 0:
        for state in to_change_coords_2:
            state[1] = 1
    ...
    if len(to_change_coords_3) != 0:
        for state in to_change_coords_3:
            state[1] = 1
    ...
    # We change paths for Robot 2 and Robot 3, and make Robot 1 follow it's original path
    
    to_change_coords_2 = []
    to_change_coords_3 = []
    
    x1, y1 = robot_1.prev_state
    x2, y2 = robot_2.prev_state
    x3, y3 = robot_3.prev_state
    if flag_2 == 0:
        # For Robot 2 only check for Robot 1
        
        if robot_1.current_state == robot_2.current_state:
            to_change_coords_2.append([[robot_1.current_state], 16])
            if robot_2.goal_state == robot_2.current_state and robot_1.goal_state == robot_1.current_state:
        ...
        if (x1 == x2 + 1 and y1 == y2) or (x1 == x2 and y1 == y2 + 1) or (x1 == x2 - 1 and y1 == y2) or (x1 == x2 and y1 == y2 - 1):
            to_change_coords_2.append([[robot_1.prev_state], 16])
    ...
    if flag_3 == 0:
        # For Robot 3 check for Robot 1 and Robot 2
        
        if robot_1.current_state == robot_3.current_state:
            to_change_coords_3.append([[robot_1.current_state], 16])
            if robot_3.goal_state == robot_3.current_state and robot_1.goal_state == robot_1.current_state:
                g_states = copy.deepcopy(goal_states)
                g_states.remove(robot_1.goal_state)
                robot_3.goal_state = min(g_states, key = lambda x : ((x[0] - robot_3.prev_state[0]) ** 2 + (x[1] - robot_3.prev_state[1]) ** 2)) 
        
        if robot_2.current_state == robot_3.current_state:
            to_change_coords_3.append([[robot_2.current_state], 16])
            if robot_3.goal_state == robot_3.current_state and robot_2.goal_state == robot_2.current_state:
                g_states = copy.deepcopy(goal_states)
                g_states.remove(robot_2.goal_state)
                robot_3.goal_state = min(g_states, key = lambda x : ((x[0] - robot_3.prev_state[0]) ** 2 + (x[1] - robot_3.prev_state[1]) ** 2)) 
        if (x1 == x3 + 1 and y1 == y3) or (x1 == x3 and y1 == y3 + 1) or (x1 == x3 - 1 and y1 == y3) or (x1 == x3 and y1 == y3 - 1):
            to_change_coords_3.append([[robot_1.prev_state], 16])
        if (x2 == x3 + 1 and y2 == y3) or (x2 == x3 and y2 == y3 + 1) or (x2 == x3 - 1 and y2 == y3) or (x2 == x3 and y2 == y3 - 1):
            to_change_coords_3.append([[robot_2.prev_state], 16])
    ...
    print(f"changed : {changed_coords_2, changed_coords_3}")
    return to_change_coords_2, to_change_coords_3
        

def main():
    current_maze = maze.Maze(1)
    
    start_states = [[7, 8], [9, 6], [9, 9]]                                    # Defining Start states
    goal_states = [[14, 8], [9, 10], [9, 5]]                                      # Defining Goal states
    """
    final_goal_states = []
    ...
    while not_at_goal_of_robot(robot_1) or not_at_goal_of_robot(robot_2) or not_at_goal_of_robot(robot_3):
        ...
        print(f"robot_2 : {robot_2.current_state}")
        print(f"robot_3 : {robot_3.current_state}")
        to_change_coords_2, to_change_coords_3 = collision_checker([robot_1, 0, flag_1], [robot_2, 1, flag_2], [robot_3, 2, flag_3], goal_states, to_change_coords_2, to_change_coords_3)
